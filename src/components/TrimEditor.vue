  ...
  <v-sheet class="pa-4" rounded="lg" border>
    ...
    <div class="d-flex flex-wrap ga-3 mt-3">
      <v-btn
        ...
        :disabled="!file || working || ffLoading"
        @click="trim"
      >
        Обрезать
      </v-btn>
  ...
  methods: {
    ...
    async ensureFFmpeg() {
      ...
      try {
        ...
        // Загружаем с CDN (как в исходном варианте)
        const baseURL = 'https://unpkg.com/@ffmpeg/core@0.12.6/dist/esm'

        const coreURL = await toBlobURL(`${baseURL}/ffmpeg-core.js`, 'text/javascript')
        const wasmURL = await toBlobURL(`${baseURL}/ffmpeg-core.wasm`, 'application/wasm')

        await this._ffmpeg.load({ coreURL, wasmURL })

        this.ffReady = true
        this.message = 'FFmpeg готов!'
      } catch (e) {
        console.error('[Trim/FFmpeg.load] error:', e)
    ...
    setStartAtCurrent() {
      this.startSec = Math.min(Math.max(0, Math.floor(this.current)), Math.floor(this.duration))
    ...
    setEndAtCurrent() {
      this.endSec = Math.min(Math.max(0, Math.ceil(this.current)), Math.ceil(this.duration))
      if (this.endSec <= this.startSec) {
